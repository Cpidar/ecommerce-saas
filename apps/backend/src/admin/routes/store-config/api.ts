 ...
 *
 * `medusa_store_id` and `tagline` are stripped — `medusa_store_id` is immutable
 * and injected server-side; `tagline` is managed via the marketing_config
...
export const saveStoreConfig = async (payload: Partial<StoreConfigInput>) => {
  ...
  // by the route from the current store scope; `tagline` is not a persisted column.
  const { medusa_store_id, tagline, ...rest } = payload as Record<string, unknown>;
