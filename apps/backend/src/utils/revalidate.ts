...
export const revalidate = async (
  ...
  path: string,
  data: { type: string; id: string; handle: string; affects_grid: boolean },
  env?: { revalidationEndpoint: string; revalidationSecret: string },
) => {
    const REVALIDATION_ENDPOINT = env?.revalidationEndpoint ?? process.env.STOREFRONT_REVALIDATION_URL ?? "";
    const REVALIDATION_SECRET = env?.revalidationSecret ?? process.env.MEDUSA_WEBHOOK_SECRET ?? "";
  // if(container){
  ...
  if (!REVALIDATION_SECRET) {
    throw new Error(
      "MEDUSA_WEBHOOK_SECRET is not set; sending webhook without secret.",
    );
  }
  ...
  try {
    ...
    console.info(
      `[subscriber] Triggered revalidation webhook for item ${data.id}`,
    );
  } catch (err) {
    throw new Error(
