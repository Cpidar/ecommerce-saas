...
export function IncredibleProductCard({ product }: ProductCardProps) {
    ...
    <Link href={`/products/${product.slug}`} className="group">
      ...
      <div className="mt-3">
        <StarRating
          ...
          size="sm"
        />
        <h3 className="mt-1 text-sm font-medium text-background group-hover:underline">
          {product.name}
        </h3>
        <div className="mt-1 flex items-center gap-2">
          ...
          {isOnSale && (
            <span className="text-xs text-muted-background line-through">
              {formatPrice(compareAtPrice, defaultVariant.currency)}
            </span>
          )}
        </div>
        {product.variants.length > 1 && (
          <p className="mt-1 text-xs text-muted-background">
            {product.variants.length} {tCommon("options")}
          </p>
        )}
      </div>
    </Link>
